...
public class Events {
            ...
            if (!hasShownUp && Minecraft.getInstance().screen == null) {
                ...
                if (modContainer != null) {
                    ...
                    if (versionCheckResult.status() == VersionChecker.Status.OUTDATED || versionCheckResult.status() == VersionChecker.Status.BETA_OUTDATED) {
                        ...
                        SuspiciousPots.LOGGER.error(References.NAME + "'s version checker failed!");
                        hasShownUp = true;
                    }
                }
            }
    ...
    private static boolean SupporterCheck(URI uri, Player player) {
        try {
            ...
            SuspiciousPots.LOGGER.error("Failed to fetch or process supporter list from URI: {}", uri, e);
            return false;
        }
    }
    ...
    private static boolean SupporterCheck(URL url, Player player) {
        try {
            ...
            SuspiciousPots.LOGGER.error("Supporter list URL not found! >>{}", url);

        } catch (Exception e) {
            SuspiciousPots.LOGGER.error("An unexpected error occurred while checking supporter list", e);
        }
